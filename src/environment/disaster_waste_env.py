...
class DisasterWasteEnv(ParallelEnv):
    ...
    def reset(
        ...
        """
        # Domain-randomised training (ASOC revision): when enabled and no
        # explicit seed is given, each episode draws a fresh realization of the
        # stochastic damage/waste processes on the *fixed* training topology.
        # This prevents the policy from overfitting to a single realization.
        # Defaults preserve the original single-realization behaviour.
        if seed is not None:
            effective_seed = seed
        elif getattr(self, "_randomize_on_reset", False):
            effective_seed = int(self._reset_rng.integers(0, 2**31 - 1))
        else:
        ...
        self._invalid_action_count = 0  # masking-mode ablation counter
        self._true_masks = {}
    ...
    def step(
        ...
        for agent in self.agents:
            ...
            # --- Masking-mode ablation: detect & handle infeasible actions ---
            # (In "hard" mode these never occur because the policy is masked.)
            _tmask = getattr(self, "_true_masks", {}).get(agent)
            if _tmask is not None and action < len(_tmask) and _tmask[action] == 0:
                self._invalid_action_count += 1
                if getattr(self, "_mask_mode", "hard") == "soft":
                    rewards[agent] -= getattr(self, "_invalid_penalty", 0.5)
    ...
    def _get_action_mask(self, agent: str) -> np.ndarray:
        ...
        for i, nbr in enumerate(neighbours):
            ...
            if health >= getattr(self, "_pass_threshold", 0.1):  # passable threshold (configurable)
                mask[i] = 1
        ...
        # --- Masking-mode ablation (ASOC revision R1.7/R3.1) ---
        # Always remember the TRUE feasibility mask so step() can count
        # constraint violations. In "hard" mode (default) the true mask is
        # exposed to the policy; in "soft"/"none" mode a permissive all-ones
        # mask is exposed so the policy may attempt infeasible actions.
        if not hasattr(self, "_true_masks"):
            self._true_masks = {}
        self._true_masks[agent] = mask
        mode = getattr(self, "_mask_mode", "hard")
        if mode == "hard":
            return mask
        permissive = np.ones(self._action_size, dtype=np.int8)
        return permissive
    ...
    def get_episode_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "invalid_action_count": int(getattr(self, "_invalid_action_count", 0)),
        }
